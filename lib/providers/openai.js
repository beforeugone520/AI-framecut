...
export async function analyzeWithOpenAI({ apiKey, model, frames, focus, meta, baseUrl }) {
  ...
  const prompt = buildAnalysisPrompt({ mode: 'frames', meta, focus });

  const userContent = [{ type: 'text', text: prompt }];
  userContent.push({ type: 'text', text: `\n以下是按时间顺序采样的 ${frames.length} 个关键帧（每帧标注其在视频中的时间戳）：` });
