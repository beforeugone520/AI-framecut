...
async function transcribeOpenAI({ apiKey, model, baseUrl, wavBuffer }) {
  ...
  if (!res.ok) {
    throw new Error(`Whisper 转写失败 (${res.status}): ${json?.error?.message || JSON.stringify(json || {}).slice(0, 200)}`);
  }
...
async function transcribeGemini({ apiKey, model, wavBuffer }) {
  const mdl = model || 'gemini-2.5-flash';

  const file = await uploadFile({ apiKey, videoBuffer: wavBuffer, mimeType: 'audio/wav', filename: 'audio.wav' });
  await waitActive({ apiKey, fileName: file.name, timeoutMs: 90000 });
...
只输出 JSON：{"segments":[{"start":0,"end":2.4,"text":"……"}]}。无人声/纯音乐则返回 {"segments":[]}。`;
  ...
  const url = `${GEMINI_BASE}/v1beta/models/${encodeURIComponent(mdl)}:generateContent?key=${encodeURIComponent(apiKey)}`;
  const res = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
  const json = await res.json().catch(() => null);
  if (!res.ok) {
    throw new Error(`Gemini 转写失败 (${res.status}): ${json?.error?.message || ''}`);
  }
