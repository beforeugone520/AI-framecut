...
export async function uploadFile({ apiKey, baseUrl, videoBuffer, mimeType, filename }) {
  ...
  if (!uploadUrl) {
    ...
    const t = await startRes.text().catch(() => '');
    if (contentType.includes('text/html') || /^<!doctype html/i.test(t.trim()) || /<html[\s>]/i.test(t)) {
      throw new Error('Gemini 文件上传初始化失败：该 API Base URL 返回了网页，不支持 Gemini Files 上传协议。请使用支持 Gemini Files API 的地址，或改用 OpenAI/Claude 抽帧模式。');
    }
    const detail = t ? `：${t.replace(/\s+/g, ' ').trim().slice(0, 160)}` : '';
    throw new Error(`Gemini 文件上传初始化失败：上游未返回 x-goog-upload-url 上传地址${detail}`);
  }
