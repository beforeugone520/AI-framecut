...
export async function analyzeWithClaude({ apiKey, model, baseUrl, frames, focus, meta, transcript }) {
  if (!apiKey) throw new Error('缺少 Claude API Key');
  const mdl = model || 'claude-sonnet-4-6';
  const endpoint = resolveEndpoint(baseUrl, BASE_URL, '/messages');
  ...
  if (!res.ok) {
    throw upstreamError('Claude 分析失败', res.status, json);
  }
