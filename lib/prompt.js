  ...
  "shots": [
    {
      ...
      "visual": "画面内容：主体、动作、环境、道具、服化道、构图与视觉重点",
      "on_screen_text": "画面中出现的文字：标题、字幕、卖点文案、品牌名、水印、按钮等，逐字照抄；没有则留空字符串",
...
3. 运动用专业运镜术语并说明幅度/速度：固定、推、拉、摇、移、跟、升降、环绕、甩、变焦（推拉 vs 变焦要区分）等；固定镜头也要写「固定」。
4. 画面内容要具体到可复刻：主体是谁/什么、在做什么、所处环境、关键道具与服化、构图与视觉重点。
5. 画面文字（on_screen_text）必须**逐字照抄**画面中出现的标题、字幕、卖点文案、品牌名等，不要改写、翻译或概括；画面中没有文字则留空字符串。
6. 音频要尽量还原：有对白/旁白则概括其内容要点，标注配乐情绪与风格、关键音效、环境声、节拍卡点。
7. 在能判断的前提下，尽量补充利于复刻的专业细节，自然融入「画面内容」「运动」或风格总结的「镜头语言」，不要新增字段；没有把握则不写，切勿编造：
   - 镜头焦段倾向：广角 / 标准 / 中长焦 / 长焦，或估计的等效焦段（写明「约 35mm，估计」之类，标注系推测）；
   - 构图法则：三分法 / 中心 / 对称 / 引导线 / 框架式 / 前景遮挡，及主体所在区域；
   - 景深：大景深（前后皆实）或小景深（背景虚化）；
   - 与下一镜的转场方式：硬切 / 叠化 / 划像 / 甩切 / 匹配剪辑 / 黑场。
8. 时长尽量精确到秒（可保留一位小数），start/end 用 m:ss 或 mm:ss 形式，duration_sec 为数字。
9. 所有文字用**简体中文**（画面文字 on_screen_text 除外，按原文照抄）。务必覆盖整段视频，不要遗漏镜头，也不要凭空编造没有依据的内容。`;

export function buildAnalysisPrompt({ mode, meta = {}, focus = '', transcript = '' }) {
  ...
  const hasTranscript = mode === 'frames' && transcript && transcript.trim();

  let modeNote = '';
  if (mode === 'frames') {
    modeNote = `输入形式：按时间顺序对视频抽取了若干关键帧（每张图片附带其在视频中的时间戳），请把这些帧理解为连续视频的采样，基于帧内容与时间戳推断镜头切分与时长。`;
    if (hasTranscript) {
      modeNote += `
另外随附了**真实音频转写**（由语音识别得到，含时间戳，见下方「音频转写」）。填写「音频」一列时：对白/旁白请**优先采用转写中对应时间段的真实文字**；配乐风格、音效、环境声等无法从转写获知的部分，再结合画面合理判断。`;
    } else {
      modeNote += `
音频无法直接听到，请结合画面中的字幕、口型、场景与上下文合理推断「音频」一列（并可注明系推断）。`;
    }
  } else {
