...
// 解析用户自定义 baseUrl（OpenAI 兼容网关），只允许 http/https，拒绝 file:// 等危险协议。
export function resolveEndpoint(baseUrl, defaultBase, path) {
  const base = (baseUrl && String(baseUrl).trim()) || defaultBase;
  let u;
  try {
    u = new URL(base);
  } catch {
    throw new Error('自定义 API Base URL 不合法');
  }
  if (u.protocol !== 'http:' && u.protocol !== 'https:') {
    throw new Error('API Base URL 仅支持 http/https 协议');
  }
